...
\texttt{kfax} funciona muy bien, pero al igual que en Windows, puedes utilizar cualquier programa que pueda mostrar archivos TIFF, tan sólo busca en la base de paqueets de tu distribución (xloadimage \emph{no} funciona).
...
\textbf{Nota:} también es posible utilizar la impresora de Acrobat Distiller
(sólo disponible en la versión completa) para genera un fichero PDF 
...
que utiliza el programa \texttt{faxcover} de HylaFAX. Revisa las siguiente ayuda 
...
Actualmente YajHFC sólo puede filtrar las columnas visibles. Por lo que tienes que añadir la columna de "Propietario" a la table en la que quieres filtrar por propietario.
...
te en cuenta que las expresiones regulares no son lo mismo que los caracteres comodín:
...
cuadro de Opciones e inténtalo de nuevo.
