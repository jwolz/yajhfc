...
checkLongOption()
{
  ...
  for (int i = 0; i < long_options.length; i++)
    {
    final String nameEndString = nextchar.substring(0, nameend);
    if (long_options[i].getName().startsWith(nameEndString))
        {
          if (long_options[i].getName().equals(nameEndString))
            {
              // Exact match found
...
getopt()
{
  ...
  String temp = null;
  final int cIndex = optstring.indexOf(c);
if (cIndex != -1)
    temp = optstring.substring(cIndex);
