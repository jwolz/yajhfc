...
public class UserDefinedConvertersSource extends FileConverterSource {
    ...
    public void addFileConvertersTo(Map<FileFormat, FileConverter> converters) {
        Map<String,String> customConverters = Utils.getFaxOptions().customFileConverters;
        
        for (FileFormat format : FileFormat.values()) {
            FileConverter defaultConverter = converters.get(format);

            // Ignore non-overridable converters
            if (defaultConverter != null && !defaultConverter.isOverridable())
                continue;

            String customConv = customConverters.get(format.name());
            if (customConv != null && customConv.length() > 0) {
                converters.put(format, new ExternalProcessConverter(customConv, defaultConverter));
            } 
        }
    }
