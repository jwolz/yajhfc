...
public class FaxJobListResolvedPhoneNumUpdater<T extends FmtItem> implements FaxJobListListener<T>, PhoneNumberMapListener {
    ...
    public void updateFaxJobs() {
        if (shouldUpdate()) {
            updateFaxJobs(jobList.getJobs(), true);
        }
    }
    ...
    public void faxJobsUpdated(FaxJobList<T> source, List<FaxJob<T>> oldJobList, final List<FaxJob<T>> newJobList) {
        if (shouldUpdate()) {
            Utils.poolExecutor.submit(new Runnable() {
                public void run() {
                    updateFaxJobs(newJobList, false);
                }
            });
        }
    }
    ...
    public void setJobList(FaxJobList<T> jobList) {
        if (jobList != this.jobList) {
            if (this.jobList != null) {
                this.jobList.removeFaxJobListListener(this);
            }
            this.jobList = jobList;
            if (jobList != null) {
                jobList.addFaxJobListListener(this);
            }
    ...
    public void columnChanged(FaxJobList<T> source, FaxJob<T> job, T column, int columnIndex, Object oldValue,
            Object newValue) {
        // Ignored
    }
